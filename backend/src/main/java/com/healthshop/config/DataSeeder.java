...
public class DataSeeder implements CommandLineRunner {
        ...
        public void run(String... args) {
                if (categoryRepository.count() > 0) {
                        ...
                        if (productRepository.count() < 10100) {
                                seedLargeScaleData();
                        }
        ...
        private void seedLargeScaleData() {
                log.info("🚀 Starting large scale data seeding to reach 10000 products and 100 categories...");

                // 1. Ensure we have 100 categories
                List<Category> categories = new ArrayList<>(categoryRepository.findAll());
                Map<String, Category> existingCats = new HashMap<>();
                for (Category c : categories)
                        existingCats.put(c.getSlug(), c);

                // Generate additional categories until we have at least 100
                String[] prefixes = { "Advanced", "Natural", "Organic", "Holistic", "Clinical", "Daily", "Essential",
                                "Premium", "Vital", "Active" };
                String[] roots = { "Heart", "Brain", "Gut", "Liver", "Kidney", "Skin", "Hair", "Bone", "Joint",
                                "Muscle", "Eye", "Ear", "Lung", "Immune", "Nerve", "Blood", "Sleep", "Energy", "Focus",
                                "Mood", "Stress", "Weight", "Detox", "Aging", "Women", "Men", "Kids", "Seniors", "Pets",
                                "Sports" };
                String[] suffixes = { "Care", "Health", "Support", "Balance", "Wellness", "Defense", "Boosters",
                                "Essentials", "Therapy", "Nutrition" };
                String[] icons = { "❤️", "🧠", "🥦", "🧪", "🦴", "💪", "👁️", "👂", "🫁", "🛡️", "🩸", "😴", "⚡", "🎯",
                                "😊", "🧘", "⚖️", "🍃", "⏳", "👩", "👨", "👶", "👴", "🐾", "🏃" };

                int catOrder = categories.size() + 1;
                Random rand = new Random(); // Use non-deterministic seed to avoid duplicate slugs on resume

                while (categories.size() < 100) {
                        ...
                        String catName = prefix + " " + root + " " + suffix;
                        String slug = catName.toLowerCase().replace(" ", "-");

                        if (!existingCats.containsKey(slug)) {
                                String icon = icons[rand.nextInt(icons.length)];
                                Category newCat = saveCategory(catName, slug,
                                                "Premium products for " + catName.toLowerCase(), icon, catOrder++);
                                categories.add(newCat);
                                existingCats.put(slug, newCat);
                        }
                }
                ...
                List<String> brands = Arrays.asList("PureLife", "VitalityX", "Nature's Way", "HerbalEssence", "BioTech",
                                "OrganicRoots", "WellnessCore", "NutraGold", "ZenHealth", "PrimeBody", "LifeForce",
                                "GreenEarth", "PeakPerformance", "HolisticCure", "MediCare+", "OptimaHealth",
                                "GlobalHealing", "EarthScience", "VitaPure", "AncientHerbals");
                List<String> adj = Arrays.asList("Advanced", "Premium", "Organic", "Natural", "Ultra", "Essential",
                                "Complete", "Max", "Pure", "Active", "Pro", "Vital", "Gold", "Super", "Daily",
                                "Nightly", "Rapid", "Gentle", "Mega", "Ultimate", "Clinical", "Herbal", "Raw", "Wild",
                                "Vegan");
                List<String> nouns = Arrays.asList("Formula", "Complex", "Support", "Booster", "Defense", "Shield",
                                "Essentials", "Extract", "Blend", "Capsules", "Drops", "Powder", "Tablets", "Softgels",
                                "Gummies", "Solution", "Therapy", "System", "Balance", "Relief", "Elixir", "Tincture",
                                "Syrup", "Oil", "Mist");
                List<String> ingredients = Arrays.asList("Vitamin C", "Zinc", "Magnesium", "Ashwagandha", "Omega-3",
                                "Probiotics", "Collagen", "Turmeric", "Ginger", "Elderberry", "Ginseng", "Melatonin",
                                "B-Complex", "Iron", "Calcium", "Vitamin D3", "Biotin", "Folic Acid", "L-Theanine",
                                "Glucosamine", "Creatine", "Whey Protein", "Pea Protein", "CoQ10", "Resveratrol");
                ...
                while (currentCount < target) {
                        try {
                                ...
                                String mainIng = ingredients.get(rand.nextInt(ingredients.size()));
                                String secIng = ingredients.get(rand.nextInt(ingredients.size()));

                                String name = adj.get(rand.nextInt(adj.size())) + " " + mainIng + " "
                                                + nouns.get(rand.nextInt(nouns.size()));
                                // ensure unique slug with loop or high random
                                String slug = name.toLowerCase().replace(" ", "-") + "-" + rand.nextInt(10000000);

                                Product p = Product.builder()
                                                .name(name)
                                                .slug(slug)
                                                .description("High-quality " + name + " by " + brand
                                                                + ". Supports your health naturally. Contains "
                                                                + mainIng
                                                                + " and " + secIng + ".")
                                                .ingredients(mainIng + ", " + secIng
                                                                + ", Cellulose, Magnesium Stearate")
                                                .benefits("Supports " + cat.getName()
                                                                + ", Boosts vitality, Enhances wellness")
                                                .price(10.0 + rand.nextInt(150))
                                                .brand(brand)
                                                .category(cat)
                                                .imageUrl(getCategoryImageUrl(cat.getSlug(), slug))
                                                .images(generateAdditionalImages(cat.getSlug()))
                                                .stock(rand.nextInt(500))
                                                .active(true)
                                                .featured(rand.nextDouble() < 0.05) // 5% chance of being featured
                                                .averageRating(2.5 + rand.nextDouble() * 2.5)
                                                .reviewCount(rand.nextInt(200))
                                                .purchaseCount(rand.nextInt(2000))
                                                .viewCount(rand.nextInt(10000))
                                ...
                                if (currentCount % 100 == 0) {
                                        log.info("... Generated " + currentCount + " products so far ...");
                                }

                                if (batch.size() >= batchSize) {
                                        log.info("Saving batch of " + batchSize + " products...");
                                        productRepository.saveAll(batch);
                                        log.info("Saved batch. Total stored: " + currentCount);
                                        batch.clear();
                                }
                        } catch (Exception e) {
                                log.error("Error generating/saving product: " + e.getMessage(), e);
                        }
                }
                if (!batch.isEmpty()) {
                        productRepository.saveAll(batch);
                        log.info("Saved final batch. Total: " + currentCount);
                }
                log.info("✅ Large scale seeding completed. Total products: " + currentCount);
        }
}
